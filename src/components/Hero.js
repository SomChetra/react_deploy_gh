import React from 'react'
import { hero } from '../data';
import { HiOutlineArrowCircleDown } from 'react-icons/hi';

const Hero = () => {
    const { title, subtitle, btnText, compText, image } = hero;

    return (
        <section className='min-h-[900px] py-12'>
            <div className='container mx-auto min-h-[900px] flex justify-center items-center'>
                <div className='flex flex-col gap-y-8 items-center justify-center text-center
                    lg:flex-row lg:gap-x-[30px] lg:gap-y-0 lg:text-left
                '>
                    {/* content */}
                    <div className="flex-1">
                        <h1 className='title mb-2 lg:mb-5'
                            data-aos='fade-down'
                            data-aos-delay='500'
                        >
                            {title}
                        </h1>
                        <p className='lead mb-5 lg:mb-10'
                            data-aos='fade-down'
                            data-aos-delay='600'
                        >
                            {subtitle}
                        </p>
                        <div className='flex items-center max-w-sm mx-auto gap-x-2 lg:max-w-full lg:mx-0 lg:gap-x-6'
                            data-aos='fade-down'
                            data-aos-delay='700'
                        >
                            <button className='btn btn-md btn-accent flex justify-center items-center lg:btn-lg lg:gap-x-4'>
                                {btnText} <HiOutlineArrowCircleDown />
                            </button>
                            <span className='text-light lg:lead lg:mb-0'>{compText}</span>
                        </div>
                    </div>
                    {/* image */}
                    <div className="flex-1" data-aos='fade-up' data-aos-delay='800'>
                        <img src={image} alt="" />
                    </div>
                </div>
            </div>
        </section>
    )
}

export default Hero;